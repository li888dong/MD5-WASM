...
Compile with emcc with:
 emcc .\MD5.cpp --no-entry -O3 -o .\md5.js -s "EXPORTED_RUNTIME_METHODS=['cwrap','ccall','allocateUTF8','allocate','stringToUTF8Array','intArrayFromString','ALLOC_NORMAL']" -s "EXPORTED_FUNCTIONS=['_malloc','_free','_md5encode']" -s WASM=1 -s ASSERTIONS=1

      源文件     不要主函数  压缩   输出文件                需要导出的环境工具参数                                                                                                                    需要到处的函数
*/
...
extern "C"
{
    char *EMSCRIPTEN_KEEPALIVE md5encode(char *input, char *res)
    {
        MD5 test;

        int len = strlen(input); // 获取输入长度
        char ch[len];

        strcpy(ch, input); // char*转char[]

        string str1 = ch; // char[]转string

        string str2 = test.encode(str1); // 对输入进行md5哈希散列

        strcpy(res, str2.data()); //将结果转到传入的引用char*
